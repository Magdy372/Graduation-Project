...
public class JwtService {
  ...
  private String buildToken(
    ...
    // Get authorities from UserDetails and add them to extraClaims
    Collection<? extends GrantedAuthority> authorities = userDetails.getAuthorities();
    
    // Ensure that role names are prefixed with "ROLE_"
    extraClaims.put("roles", authorities.stream()
            .map(authority -> authority.getAuthority())  
            .collect(Collectors.toList()));
            
    // Build the JWT token with additional claims and expiration time
    return Jwts.builder()
            .setSubject(userDetails.getUsername()) // Set the subject (email or username)
            .setClaims(extraClaims)  // Add the extra claims here 
